...
## Security: ротация секретов

### Что внедрено

- JWT поддерживает «ключи в списке»:
  - подпись новым ключом (первый в `JWT_SECRET_KEYS`);
  - проверка всеми ключами списка.
- HMAC терминалов берётся из `terminal.secret_hash`.
- Появился утилитарный скрипт: `scripts/rotate_secrets.py`.

### Зачем

- уменьшить риск при утечке ключа;
- иметь регулярную операционную процедуру (а не разовые ручные правки);
- поддержать плавную JWT-ротацию без немедленного logout.

### JWT: рабочая схема ротации

1. Сгенерировать новый ключ.
2. Установить `JWT_SECRET_KEYS=<new>,<old>`.
3. Перезапустить backend.
4. После истечения TTL токенов (`jwt_expire_minutes`) убрать старый ключ:
   - `JWT_SECRET_KEYS=<new>`
5. Перезапустить backend.

### HMAC терминалов: рабочая схема ротации

1. Выполнить:
   - `cd backend_full && python scripts/rotate_secrets.py --apply-terminal-secrets`
2. Забрать вывод `TERMINAL_ID=SECRET`.
3. Обновить секреты на терминалах.
4. Проверить HMAC-запросы терминалов.

Примечание:
- `--dry-run` не подходит как источник финальных секретов (каждый запуск генерирует новый набор).

### Пароли БД: рабочая схема ротации

1. Выполнить `ALTER ROLE ... WITH PASSWORD ...` в PostgreSQL.
2. Обновить `.env` (`POSTGRES_PASSWORD`, `SIMPLE_POSTGRES_PASSWORD`).
3. Перезапустить backend/воркеры.
4. Проверить доступ к БД и миграции.

### Быстрая генерация секретов

- `cd backend_full && python scripts/rotate_secrets.py`
- выводит шаблон новых значений для `.env`.
